package com.kayrasolutions.aem.foundation.core.link.builders.factory;
...
import com.kayrasolutions.aem.foundation.core.constants.PropertyConstants;
import com.kayrasolutions.aem.foundation.core.link.builders.impl.DefaultLinkBuilder;
...
public final class LinkBuilderFactory {
	...
	/**
	 * Get a builder instance for an existing <code>Link</code>. The path,
	 * extension, title, and target are copied from the link argument.
	 *
	...
	public static LinkBuilder forLink(final Link link) {
		...
		return new DefaultLinkBuilder(link.getPath(), null).setExtension(link.getExtension()).setTitle(link.getTitle())
				.setTarget(link.getTarget());
	}

	/**
	 * Get a builder instance for a page. If the page contains a redirect, the
	 * builder will contain the redirect target rather than the page path.
	 *
	 * @param page page
	...
	/**
	 * Get a builder instance for a page using the specified title type on the
	 * returned builder.
	 *
	 * @param page      page
	...
	/**
	 * Get a builder instance for a resource using the mapped path on the returned
	 * builder.
	 *
	 * @param resource resource
	 * @param mapped   if true, link path will be mapped through resource resolver
